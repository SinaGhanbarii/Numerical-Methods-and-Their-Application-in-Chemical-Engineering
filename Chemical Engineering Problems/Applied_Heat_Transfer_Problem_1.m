...
Coefficient_Matrix = [ (-1/Res_1) + (-1/Res_12) + (-1/Res_13),  (1/Res_13),  0;
    (1/Res_12),  (1/Res_23),  0;
...
Regular_Answers = Coefficient_Matrix \ Constants_Matrix;
...
for m = 1:Iter_num
    for i = 1:length(Constants_Matrix)
        ...
        total_sum = Coefficient_Matrix(i, :) * Gauss_Seidel_Answers - Coefficient_Matrix(i, i) * Gauss_Seidel_Answers(i);

        % Update the solution vector component x(i) with relaxation factor
        Gauss_Seidel_Answers(i) = (1 - omega) * Gauss_Seidel_Answers(i) + (omega / Coefficient_Matrix(i, i)) * (Constants_Matrix(i) - total_sum);
    end
end
...
n = length(Coefficient_Matrix);
Gauss_Jordan_Answers = zeros(n,1);
for i=1:n-1
    if Coefficient_Matrix(i,i)==0
        [m, k]=max(abs(Coefficient_Matrix(i+1:end,i)));
        Coefficient_Matrix(i,:)=Coefficient_Matrix(i,:)+Coefficient_Matrix(i+k,:);
        Constants_Matrix(i)=Constants_Matrix(i)+Constants_Matrix(i+k);
    end
end
M=[Coefficient_Matrix,Constants_Matrix];
